...
function getInfosPerso(message) {
    (async function () {
        ...
        try {
            ...
            console.log("infos demandées par :" + charVerif.discordName);
            if (charVerif === null)
            ...
            for (var power = 0; power < 6; power++)
            {
                for (var type = 0; type < 4; type++)
                {
                    ...
                    if (isChanged === 1)
                    {
                        ...
                        if (compteur % 5 > 0) {
                            let itemLoot = await col.findOne({itemType: listType[type], itemPower: (power)});
                            const item1 = {
                                itemID: null,
                                characterName: message.author.username,
